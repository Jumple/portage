...
# $Header: /var/cvsroot/gentoo-x86/net-libs/pjsip/pjsip-1.14.ebuild,v 1.1 2012/04/29 19:04:34 elvanor Exp $
...
src_install() {
	DESTDIR="${D}" emake -j1 install || die "emake install failed."
	...
	if use python; then
		...
		emake -j1 || die "python emake failed."
		DESTDIR="${D}" emake -j1 install || die "python emake install failed."
		popd
	fi
